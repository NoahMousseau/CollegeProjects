/*==========================================================
 ...
 Date:    	10/25/2022
 Description: This program contains three functions, which (after being given an array)
              is able to alter values within that array, print each value in that array,
              and print the sum of values in that array
...
void alterGameScore(int pArr[], int gameNumber) {
    ...
    int k; //Declares a value k to be used for user input
    cin>>k;
    pArr[gameNumber] = pArr[gameNumber] + k; //Adds k to the specified value within an array
}

void printScores(int pArr[], int gameMAX) {
	for (int i = 0; i<gameMAX; i++) { //Loops through the array to print each value within
	    cout<<pArr[i]<<" ";
	}
}
...
int totalScore(int pArr[], int gameMAX) {
    ...
    int sum = 0;
    for (int i = 0; i<gameMAX; i++) { //Loops through the array to find the sum and print it
        sum = sum + pArr[i];
    }
    return sum;
}
